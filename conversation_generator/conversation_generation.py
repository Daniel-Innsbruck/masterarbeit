...
USE_CACHE = True            # shoudl cache be read?
CACHE_MODE = "all"
CACHE_TAU = 0.2
...
FIXED_ROOT_IDS = [
    "bfeeeccc-65bf-4dfd-bff6-483a1d9e6fe1"
]

## Fail-Fast parameters
MAX_RETRIES = 3  # Max retries per turn (independent of n)
MAX_CONSECUTIVE_FAILS = 8 # max number of complete conversation restarts before qa-gernation ends early (avoid infinite loop)

# dialog configs
n = 5             # Target number of turns per conversation
max_conversations = 30 # number conversations'

GLOBAL_OFFSET = 1

# Logging & Output
output_file = "./data/bfeeeccc-65bf-4dfd-bff6-483a1d9e6fe1.jsonl"#"./data/45ea68d7-6e88-4ba9-a8d3-cb2366b1651a.jsonl"
log_file = "./data/bfeeeccc-65bf-4dfd-bff6-483a1d9e6fe1.log"#"./data/45ea68d7-6e88-4ba9-a8d3-cb2366b1651a.log"
metrics_file = "./data/cache_metrics_bfeeeccc-65bf-4dfd-bff6-483a1d9e6fe1.jsonl"#"./data/cache_metrics_45ea68d7-6e88-4ba9-a8d3-cb2366b1651a.jsonl"
...
def _build_active_context_string_for_validator(active_chunks):
    """Builds a structured string from all currently active chunks"""
...
def get_follow_up_question(answer, active_chunks, expanding_context = "", max_retries=MAX_RETRIES):
    ...
    for attempt in range(max_retries + 1):
        ...
        if validation and validation['correct']:
            return response, attempt, total_val_in, total_val_out, total_gen_time, total_val_time
...
def generate_conversation():
    ...
    while counter < iterations:
        ...
        if USE_CACHE and FIXED_ROOT_IDS:
            root_id = FIXED_ROOT_IDS[0]#[counter]
        ...
        while turn_idx < n:
                ...
                if cached_bundle:
                    ...
                    if CACHE_SAFEGUARD:
                        ...
                        if validation and validation['correct']:
                            ...
                            validator_passed = False
                            cache_rejection_reason = reason
                    else:
                        print("Safeguard disabled. Reusing cached follow-up.")
                        next_question = proposed_question
                        cache_accepted = True

                    # history injection in case of cache hit
                    if cache_accepted:
            ...
            # =========================================================
            #  Follow-Up generation in case cache was dismissed or cache is disabled (max. 3 fails)
            # =========================================================
            if not cache_accepted:
                result = get_follow_up_question(answer, active_chunks, expanding_context=expanding_context,
                                                max_retries=MAX_RETRIES)

                if result is None or result[0] is None:
                    gen_tokens_in, gen_tokens_out = model.get_and_reset_turn_tokens()
                    if result is not None:
                        _, turn_retries, fu_val_in, fu_val_out, fu_gen_time, fu_val_time = result
                        val_tokens_in += fu_val_in
                        val_tokens_out += fu_val_out
                        time_spent_generating += fu_gen_time
                        time_spent_validating += fu_val_time
                    total_turn_time = time.time() - turn_start_time
                    metrics_entry = {
                        "conversation_id": counter + GLOBAL_OFFSET,
                        "turn_index": turn_idx,
                        ...
                        "rag_time_sec": round(time_spent_rag, 2)
                    }
                    with open(metrics_file, 'a', encoding='utf-8') as f:
                        f.write(json.dumps(metrics_entry) + "\n")

                    print(
                        f"  Follow-Up Generation failed after {MAX_RETRIES} retries at turn {turn_idx + 1}. Metrics logged before restart.")
                    conversation_failed = True
                    break

                next_question, turn_retries, fu_val_in, fu_val_out, fu_gen_time, fu_val_time = result
                val_tokens_in += fu_val_in
                val_tokens_out += fu_val_out
                time_spent_generating += fu_gen_time
                time_spent_validating += fu_val_time
